...
const PassThrough = stream_1.PassThrough;
 ...
 */
const CPU_COUNT = Math.max(require$$1.cpus().length, 1);
...
function getEnvData(configDir, options) {
    ...
    var cwd = configDir, globConfig = {
        cwd: cwd,
        root: "/",
    }, PRJ_ENV = process.env.PRJ_ENV || process.env.NODE_ENV || "production", defaultEnvPath = globby.sync(options.defaultFileName + ".*(yaml|yml)", globConfig)[0] || "";
    if (!defaultEnvPath) {
        throw new Error("config dir must include default.yml or default.yaml");
    }
    var defaultEnv = parseData(defaultEnvPath, cwd), envFile = globby.sync(((options.files && options.files[0]) || "production") + ".*(yaml|yml)", globConfig)[0] || "", env = parseData(envFile, cwd), envMergeData = lodash_merge({
        PRJ_ENV: PRJ_ENV,
        VERSION: version,
    }, defaultEnv, env);
